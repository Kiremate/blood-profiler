...
ProbeResult UdpProbe::execute() {
    ...
    try {
        ...
        // Resolve endpoint
        auto endpoints = resolver.resolve(boost::asio::ip::udp::v4(), host_, std::to_string(port_));
        ...
        while (std::chrono::high_resolution_clock::now() < timeout_time) {
            bytes_received = socket.receive_from(
                boost::asio::buffer(response_buffer), sender_endpoint, 0, ec);
        ...
        if (result.success) {
            result.additional_info = "Received " + std::to_string(bytes_received) + " bytes";
        }
