...
public:
    ...
    void addHttpsProbe(const std::string& host, int port, const std::string& sni_override = "");
    void addUdpProbe(const std::string& host, int port);
    
    std::vector<ProbeResult> runProbes();
    void saveResults(const std::vector<ProbeResult>& results, const std::string& filename);
